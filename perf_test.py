...
clients = {
    sellerClient: ["get_seller_rating", "sell_item", "list_items", "remove_item", "change_item_price"],
    buyerClient: ["search", "add_items_to_cart", "make_purchase", "get_seller_rating_by_id", "get_purchase_history"]
}
...
results = {method: client.testEach(client.__getattribute__(method)) for client, methods in clients.items() for method in methods}
...
for method, res_time in results.items():
    print(f"{method}: {res_time}")
print("============================")
